...
def publish_reel(video_public_url: str, caption: str) -> str:
    ...
    if not create.ok:
        print(f"Instagram media-container creation failed ({create.status_code}): {create.text}")
    ...
    for _ in range(30):
        ...
        if not r.ok:
            print(f"Instagram container status check failed ({r.status_code}): {r.text}")
        r.raise_for_status()
    ...
    if not publish.ok:
        print(f"Instagram publish failed ({publish.status_code}): {publish.text}")
